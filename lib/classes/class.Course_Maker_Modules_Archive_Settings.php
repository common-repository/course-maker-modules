...
class Course_Maker_Modules_Archive_Settings {
	...
	static function register_metaboxes( $pagehook ) {
		...
		if ( 'modules_page_genesis-cpt-archive-modules' !== $pagehook ) {
			return;
		}
	...
	/**
	 * Builds the HTML output for the module archive settings.
	 *
	 * @access public
	...
	function __construct( $pagehook ) {
		...
		$this->settings_field = GENESIS_CPT_ARCHIVE_SETTINGS_FIELD_PREFIX . 'modules';

		add_meta_box( 'post_per_page', __( 'Items Per Page', 'course-maker-modules' ), array( $this, 'posts_per_page_metabox' ), $pagehook, 'main', 'low' );
	...
	<tr valign="top">
		...
		<td><input name="%1$s[%2$s]" type="number" step="1" min="1" id="%1$s-%2$s" value="%5$s" class="small-text">%4$s</td>
			...
			$this->settings_field,
			$key,
			__( 'Archives show at most', 'course-maker-modules' ),
			__( ' module items'     , 'course-maker-modules' ),
